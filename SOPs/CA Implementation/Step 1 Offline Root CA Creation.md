[[View PDF Version]](<./SOPs/CA Implementation/PDF - Step 1 Offline Root CA Creation>)
...
- In a new file **C:WindowsCAPolicy.inf**, have the following content:
...
- For **Validity Period**, match the time to the renewal validity period listed in **C:WindowsCAPolicy.inf**
...
- With the **CRL Distribution Point (CDP)** extension selected, have only the following entries available:
  ...
  - [Change the FQDN] <http://INTERCA.[INTERNAL-ORG].local/CertEnroll/<CaName><CRLNameSuffix><DeltaCRLAllowed>.crl>
...
- With the **Authority Information Access (AIA)** extension selected, have only the following entries available:
  ...
  - [Change the FQDN] <http://INTERCA.[INTERNAL-ORG].local/CertEnroll/<CaName><CertificateName>.crt>
...
- Both the root certificate file (.cer) and the root revocation list file (.crl) should be found under **C:WindowsSystem32CertSrvCertEnroll**
...
- **IF wish to specify the validity period of the Inter CA**
  ...
  - In admin CMD: certutil -setreg CAValidityPeriodUnits (**number**)

  - In admin CMD: certutil -setreg CAValidityPeriod "Years"
